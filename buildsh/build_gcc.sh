...
DESTDIR=/home/lixq/toolchains/gcc-${ver}
[[ -n "$1" ]] && DESTDIR="$1"

need_exit=no
if [[ ! -f /home/lixq/35share-rd/srcgcc-${ver}.tar.gz ]]; then
...
if [[ "${DESTDIR}" == */gcc* ]]; then
    ...
    . "$(dirname "${BASH_SOURCE[0]}")/set_build_env.sh" toolset
fi
...
../configure --disable-multilib || exit 1
make -s -j"$(nproc)"
